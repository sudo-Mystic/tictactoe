...
export default function TicTacToePage() {
  ...
  const [gamePhase, setGamePhase] = useState<"menu" | "waiting" | "playing" | "finished">("menu")
    ...
    <div className="min-h-screen bg-gray-950 text-white p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
            ...
            <CardContent className="space-y-4">
              ...
              {gamePhase === "waiting" && (
                <div className="text-center space-y-4">
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <Button variant="ghost" size="sm" onClick={copyRoomCode} className="h-6 w-6 p-0 hover:bg-gray-800">
                  ...
                  <p className="text-sm text-gray-400">
                    You are playing as <span className="font-bold text-white">{playerSymbol}</span>
                  </p>
                </div>
              )}
              ...
              {(gamePhase === "playing" || gamePhase === "finished") && (
                <div className="space-y-4">
                      ...
                      {gameState.winner ? (
                        <div
                          className={`text-lg font-bold ${
                            gameState.winner === playerSymbol ? "text-green-400" : "text-red-400"
                          }`}
                        >
                          {gameState.winner === playerSymbol ? "🎉 You Won!" : "😔 You Lost!"}
                        </div>
                      ) : gameState.draw ? (
                        <div className="text-lg font-bold text-yellow-400">🤝 It's a Draw!</div>
                      ) : (
                        <div
                          className={`text-lg ${
                            gameState.turn === playerSymbol ? "text-green-400" : "text-yellow-400"
                          }`}
                        >
                          {gameState.turn === playerSymbol ? "Your Turn" : "Opponent's Turn"}
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {gamePhase === "finished" && (
                    <Button onClick={resetGame} className="w-full bg-purple-600 hover:bg-purple-700 transition-colors">
              ...
              {connectionStatus !== "connected" && (
                <Button onClick={connectWebSocket} className="w-full bg-gray-700 hover:bg-gray-600 transition-colors">
                  Reconnect
                </Button>
              )}
            </CardContent>
          ...
          <Card className="bg-gray-900 border-gray-800">
            <CardHeader>
              <CardTitle className="text-white text-center">Game Board</CardTitle>
            </CardHeader>
            <CardContent>
                  ...
                  return (
                    <button
                      ...
                      className={`
                        ...
                        ${canPlay ? "hover:bg-gray-700 hover:border-gray-600 hover:scale-105 cursor-pointer" : "cursor-not-allowed"}
                        ${isLastMove ? "animate-pulse bg-gray-700" : ""}
                        ${cellValue === "X" ? "text-blue-400" : cellValue === "O" ? "text-red-400" : "text-gray-500"}
                      `}
                    >
                      <span className={`transition-all duration-300 ${cellValue ? "scale-100" : "scale-0"}`}>
                        {cellValue || ""}
                      </span>
                    </button>
                  )
              ...
              {gamePhase === "menu" && (
                <div className="text-center mt-6 text-gray-400">Create or join a game to start playing</div>
              )}
            </CardContent>
          </Card>
        </div>
